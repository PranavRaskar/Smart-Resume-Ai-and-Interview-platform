...
from voice_interview.evaluator import evaluate_answer
from voice_interview.voice_engine import transcribe_audio
...
class ResumeApp:
    def __init__(self):
        ...
        self.pages = {
            ...
            "🎤 AI MOCK INTERVIEW": self.render_voice_interview,
        }
    ...
    def render_voice_interview(self):
        apply_modern_styles()
        ...
        if q_index >= len(questions):
            ...
            total_tech = sum(r["technical_score"] for r in results)
            total_comm = sum(r["communication_score"] for r in results)
            total_conf = sum(r["confidence_score"] for r in results)

            st.metric("Technical Score", total_tech)
            st.metric("Communication Score", total_comm)
            st.metric("Confidence Score", total_conf)

            for i, r in enumerate(results):
                st.write(f"Q{i+1} Feedback: {r['feedback']}")
